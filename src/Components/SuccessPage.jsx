...
export default function SuccessPage() {
  ...
  useEffect(() => {
    ...
    async function fetchCrossSelling() {
      try {
        ...
        console.log("Fetching cross-sell products...");
        ...
        console.log("Cross-sell response status:", res.status);
        ...
        const data = await res.json();
        console.log("Cross-sell data:", data);

        if (!data?.status) {
          throw new Error("API returned status=false");
        }

        // Set products from the correct field
        const productList = data.cross_sell_products || [];
  ...
  const ProductCard = ({ item }) => (
    <div className="flex-shrink-0 w-[280px] border rounded-2xl p-5 shadow hover:shadow-lg transition bg-white mx-3">
      ...
      <button
        onClick={() => {
          window.location.href = item.live_link;
        }}
